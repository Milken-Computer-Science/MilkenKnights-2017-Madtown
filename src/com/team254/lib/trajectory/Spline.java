...
public class Spline {
    ...
    public static boolean reticulateSplines(WaypointSequence.Waypoint start,
        ...
        return reticulateSplines(start.x, start.y, start.theta, goal.x, goal.y, goal.theta, result,
            type);
    }

    public static boolean reticulateSplines(double x0, double y0, double theta0, double x1,
        double y1, double theta1, Spline result, Type type) {
        ...
        if (almostEqual(Math.abs(theta0_hat), Math.PI / 2) || almostEqual(Math.abs(theta1_hat),
            Math.PI / 2)) {
            return false;
        }
    ...
    public double getPercentageForDistance(double distance) {
        ...
        if (arc_length != last_arc_length) {
            interpolated +=
                ((distance - last_arc_length) / (arc_length - last_arc_length) - 1) / kNumSamples;
        }
        return interpolated;
    }
    ...
    public double[] getXandY(double percentage) {
        ...
        double y_hat =
            (a_ * x_hat + b_) * x_hat * x_hat * x_hat * x_hat + c_ * x_hat * x_hat * x_hat
    ...
    public double angleAt(double percentage) {
        double angle =
            ChezyMath.boundAngle0to2PiRadians(Math.atan(derivativeAt(percentage)) + theta_offset_);
        return angle;
    }
