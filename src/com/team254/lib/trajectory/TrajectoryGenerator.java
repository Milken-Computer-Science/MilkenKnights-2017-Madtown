...
public class TrajectoryGenerator {
     ...
     * <p>
     * Read the notes on each of the Strategies defined above, as certain arguments are ignored for
     * some strategies.
     *
     ...
     */
    public static Trajectory generate(Config config, Strategy strategy, double start_vel,
        double start_heading, double goal_pos, double goal_vel, double goal_heading) {
        ...
        if (strategy == StepStrategy) {
            ...
            // Round down, meaning we may undershoot by less than max_vel*DELTA_T.
            ...
            double x_rampdown =
                adjusted_max_vel * t_rampdown - .5 * config.max_acc * t_rampdown * t_rampdown;
            double x_cruise = goal_pos - x_rampdown - x_rampup;

            // The +.5 is to round to nearest
            int time =
                (int) ((t_rampup + t_rampdown + x_cruise / adjusted_max_vel) / config.dt + .5);
            ...
            double impulse =
                (goal_pos / adjusted_max_vel) / config.dt - start_vel / config.max_acc / config.dt
                    + start_discount + end_discount;
            traj = secondOrderFilter(f1_length, 1, config.dt, start_vel, adjusted_max_vel, impulse,
                time, TrapezoidalIntegration);

        } else if (strategy == SCurvesStrategy) {
            // How fast can we go given maximum acceleration and deceleration?
            double adjusted_max_vel = Math.min(config.max_vel,
                (-config.max_acc * config.max_acc + Math.sqrt(
                    config.max_acc * config.max_acc * config.max_acc * config.max_acc
                        + 4 * config.max_jerk * config.max_jerk * config.max_acc * goal_pos)) / (2
                    * config.max_jerk));
            ...
            traj = secondOrderFilter(f1_length, f2_length, config.dt, 0, adjusted_max_vel, impulse,
                time, TrapezoidalIntegration);
        ...
        for (int i = 0; i < traj.getNumSegments(); ++i) {
            traj.segments_[i].heading =
                start_heading + total_heading_change * (traj.segments_[i].pos) / traj.segments_[
                    traj.getNumSegments() - 1].pos;
        }

        return traj;
    }

    private static Trajectory secondOrderFilter(int f1_length, int f2_length, double dt,
        double start_vel, double max_vel, double total_impulse, int length,
